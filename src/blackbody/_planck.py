...
def _spectral_radiant_sterance_nu(
    c1: float, c2: float, T: NDArray[np.float64], nu: NDArray[np.float64]
):
    x = nu / T
    xmax = EMAXEXP / c2
    x[x > xmax] = xmax
...
def _spectral_photon_sterance_nu(
    c1: float, c2: float, T: NDArray[np.float64], nu: NDArray[np.float64]
):
    x = nu / T
    xmax = EMAXEXP / c2
...
def _spectral_radiant_sterance_lambda(
    c1: float, c2: float, T: NDArray[np.float64], xlambda: NDArray[np.float64]
):
    x = 1 / (xlambda * T)
    xmax = EMAXEXP / c2
    x[x > xmax] = xmax
...
def _spectral_photon_sterance_lambda(
    c1: float, c2: float, T: NDArray[np.float64], xlambda: NDArray[np.float64]
):
    x = 1 / (xlambda * T)
    xmax = EMAXEXP / c2
...
def _spectral_radiant_sterance_sigma(
    c1: float, c2: float, T: NDArray[np.float64], sigma: NDArray[np.float64]
):
    x = sigma / T
    xmax = EMAXEXP / c2
    x[x > xmax] = xmax
...
def _spectral_photon_sterance_sigma(
    c1: float, c2: float, T: NDArray[np.float64], sigma: NDArray[np.float64]
):
    x = sigma / T
    xmax = EMAXEXP / c2
...
def _integrated_radiant_sterance_nu(
    c1: float, c2: float, T: NDArray[np.float64], nu: NDArray[np.float64]
):
    return c1 / c2**4 * T**4 * _planck_integral_3(c2 * nu / T)


def _integrated_photon_sterance_nu(
    c1: float, c2: float, T: NDArray[np.float64], nu: NDArray[np.float64]
):
    return c1 / c2**3 * T**3 * _planck_integral_2(c2 * nu / T)


def _integrated_radiant_sterance_lambda(
    c1: float, c2: float, T: NDArray[np.float64], xlambda: NDArray[np.float64]
):
    return c1 / c2**4 * T**4 * _planck_integral_3(c2 / xlambda / T)


def _integrated_photon_sterance_lambda(
    c1: float, c2: float, T: NDArray[np.float64], xlambda: NDArray[np.float64]
):
    return c1 / c2**3 * T**3 * _planck_integral_2(c2 / xlambda / T)


def _integrated_radiant_sterance_sigma(
    c1: float, c2: float, T: NDArray[np.float64], sigma: NDArray[np.float64]
):
    return c1 / c2**4 * T**4 * _planck_integral_3(c2 * sigma / T)


def _integrated_photon_sterance_sigma(
    c1: float, c2: float, T: NDArray[np.float64], sigma: NDArray[np.float64]
):
    return c1 / c2**3 * T**3 * _planck_integral_2(c2 * sigma / T)


PLANCK_DISTRIBUTIONS: dict[
    tuple[str, str],
    Callable[[float, float, NDArray[np.float64], NDArray[np.float64]], NDArray[np.float64]],
] = {
...
INTEGRATED_PLANCK_DISTRIBUTIONS: dict[
    tuple[str, str],
    Callable[[float, float, NDArray[np.float64], NDArray[np.float64]], NDArray[np.float64]],
] = {
